...
func _confirm_delete():
    ...
    _active_dialog.content_text = "是否从本地[color=RED]删除谱面文件[/color]？[br][b]※注意：[/b][u]从Steam下载的[/u]创意工坊谱面在此处删除后会[color=BLUE]使Rizline报“谱面文件损坏”错误[/color]。如需彻底删除/恢复请在Steam中[b]取消订阅[/b]并[b]在Steam的“属性-创意工坊”中手动删除文件[/b]。收藏信息也会一并移除。您可以在系统回收站中找到被删除的谱面文件夹。"
